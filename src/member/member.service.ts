...
import { LogTokenCreateInput } from 'src/log_token/dto/log-token-create.input';
import { SourcesGrantService } from 'src/sources_grant/sources-grant.service';
import { SourcesGrantInput } from 'src/sources_grant/dto/sources-grant.input';
...
export class MemberService {
  ...
  async searchMember(input: MemberSearchInput): Promise<any> {
    ...
    const memberEntities = await this.memberRepository.find({
      where: [
        { name: Like(`%${input.word}%`), grant_id: input.grant },
        { surname: Like(`%${input.word}%`), grant_id: input.grant },
        { username: Like(`%${input.word}%`), grant_id: input.grant },
      ],
      order: {
        name: 'ASC',
        surname: 'ASC',
